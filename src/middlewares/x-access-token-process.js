...
export const decodedToken = (req, res, next) => {
  ...
  if(!token) return res.status(403).json({ ok: false, error: 403, message: "Access TOKEN not received." });
  ...
  jwt.verify(token, process.env.SECRET_WORD, async (err, decoded) => {      
    ...
    if(!user) return res.status(404).json({ ok: false, error: 404, mensaje: 'Invalid access - Information not found.' });
